...
export const AdminDashboard: React.FC = () => {
  ...
  const { isCategoryManager } = useAdminAuth();
  ...
  const fetchData = async () => {
    try {
      // Un category_manager n'a pas accès à /admin/analytics (données globales hors de son
      // périmètre catégorie) : on ne l'appelle pas pour lui, sinon le Promise.all échoue
      // entièrement et casse aussi l'affichage des stats scopées, qui elles fonctionnent.
  ...
  return (
      <div className="space-y-6">
        ...
        {/* Stats Cards */}
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${isCategoryManager ? 'lg:grid-cols-2' : 'lg:grid-cols-4'} gap-4 lg:gap-6 mb-6 lg:mb-8`}>
          {!isCategoryManager && (
            <StatsCard
        ...
        {/* Analytics Charts (hors périmètre category_manager, données globales) */}
        {!isCategoryManager && (
        ...
        {/* Recent Users (hors périmètre category_manager) */}
        {!isCategoryManager && (
